...
ggplot(datos_surv, aes(x = edad)) +
  ...
  geom_vline(
    aes(xintercept = median(edad, na.rm = TRUE)),
    linetype = "dashed",
    linewidth = 0.8,
    color = "black"
  ) +
  ...
  labs(
    title = "Distribución de edad al diagnóstico",
    x = "Edad (años)",
    y = "Frecuencia"
  ) +
...
ggplot(
  ...
  aes(
    x = "",
    y = tamano_tumoral_al_dx_por_tc_diametro_mayor_en_cm
  )
) +
  ...
  labs(
    title = "Distribución del tamaño tumoral al diagnóstico",
    y = "Tamaño tumoral (cm)",
    x = NULL
  ) +
...
ggplot(
  ...
  labs(
    title = "Distribución de CA 19-9 al diagnóstico",
    y = "CA 19-9 (U/mL)",
    x = NULL
  ) +
...
#-------------------------------------------------
#Tabla 1. Características basales de los pacientes (N = 140)
#-------------------------------------------------
...
#-------------------------------------------------
# Tabla 2. Distribución de metástasis al diagnóstico (N = 74)
#-------------------------------------------------
...
#-------------------------------------------------
# Tabla A3. Características de los pacientes operados con intención curativa (N = 28)
#-------------------------------------------------
datos_qx <- datos_surv %>%
...
tabla_qx %>%
  kable(
    col.names = c("Variable","Categoría / Resumen","n (%) o estadístico"),
    align = c("l","l","r"),
    caption = paste0("Tabla A3. Características de los pacientes operados con intención curativa (N = ", N_qx, ")"),
    booktabs = TRUE
  ) %>%
  kable_styling(
    full_width = FALSE,
